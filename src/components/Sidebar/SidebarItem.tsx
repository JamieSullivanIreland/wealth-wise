...
import { IMenuChildren, IMenuItem } from '@/types/menu';
...
const SidebarItem = ({ item, pageName, setPageName }: IProps) => {
  const handleClick = () => {
  ...
  const isActive = (item: IMenuItem) => {
    ...
    if (item.children) {
      return item.children.some((child: IMenuItem) => isActive(child));
    }
    return false;
  };
